...
import pandas as pd
from free_function import read_grd_rain, read_grd_temp, csv2shp
...
###########################################################
temp_range = list(range(int(maxtemp_start), int(maxtemp_stop)+1))
#######################################################
if rainfall == 'y':
...
#for file in files:
 ...
 #       year = file[-8:-4]
  #      print(year)
  #      df_rain = read_grd_rain(file, year, result_dir)
        
  #  csv2shp(df_rain, result_dir, year, 'rain')
...
for file in files_max:
    ...
    equ_file = os.path.join(curr_wd, 'equation_'+str(year)+'.csv')
    equ_file_csv = open(equ_file, 'w', newline='')
    ...
    df_equ.insert(0, 'Latitude', df_min['Latitude'][0:961])
    df_equ.insert(1, 'Longitude', df_min['Longitude'][0:961])
    ...
    for i in range(len(df_header)):
        ...
        ##########################################################################
        ### condition
        diff[np.absolute(diff)>700] = 0
        diff = np.sqrt((diff))
        diff = np.nan_to_num(diff)
        #############################################################################
        
        ra = 10
        equ = 0.0022*ra*(diff)*(c + 17.8)
        
        header = list(df_equ)
        df_equ.insert(len(header),  'day_'+str('%s'%(i+1)), equ)
        
#    df_mean.to_csv(mean_file_csv, index=False)
#    df_equ.to_csv(equ_file_csv, index=False)
    
    csv2shp(df_equ, result_dir, year, 'temp')
    ###############################################################################
